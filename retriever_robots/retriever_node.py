...
class RetrieveNode(Node):
    ...
    def pose_controller_reverse(self, target_pose: Pose) -> Twist:
        ...
        # only works if the robot is initialized under the catwalk facing the build area (and the stockpiles are where they are currently)
        if self.curr_pose.pose.position.x < target_pose.position.x:
            t = Twist()
            t.linear.x = -0.2
            return t

        return self.pose_controller(target_pose)
